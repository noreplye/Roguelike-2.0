public class Quest
{
    ...
    public static void MainQuest2(map current_map)
    {
        ...
        Console.ReadKey();
        ColorOfName("- Пора бы мне представиться. Я тот кто может тебе дать второй шанс. Я помогу тебе выбраться", ConsoleColor.Yellow);
        Console.ReadKey();
        ColorOfName("- Но почему?", ConsoleColor.Green);
        Console.ReadKey();
        ColorOfName("- Ты проделал огромную работу, это подземелье что-то среднее между адом и человечеством.", ConsoleColor.Yellow);
        Console.ReadKey();
        ...
        Console.ReadKey();
        ColorOfName("- Иди к следующей двери я предупрежу тебя сразу, там ты встретишь самого опасного врага", ConsoleColor.Yellow);
        Console.ReadKey();
